...
// jmarshall
idCVar g_casUpscale("g_casUpscale", "1", CVAR_BOOL, "upscale post process");
idCVar g_casPixelScale("g_casPixelScale", "256", CVAR_FLOAT, "upscale post process");
// jmarshall end
...
void hhPlayerView::SingleView( idUserInterface *hud, const renderView_t *view ) {
	...
	static idVec2 shiftScale = idVec2(1, 1);
...
// jmarshall - CAS
	if (g_casUpscale.GetBool()) {
		const idMaterial *mtr = nullptr;
		
		if(renderSystem->GetScreenWidth() == 1280)
			mtr = declManager->FindMaterial("postprocess/casupscale_1280", false);
		else if(renderSystem->GetScreenWidth() == 1024)
			mtr = declManager->FindMaterial("postprocess/casupscale_1024", false);

		renderSystem->CaptureRenderToImage("_scratch");
		if (!mtr) {
			common->Printf("You are not running a supported resolution. Please use r_mode 5 or r_mode 7, side effect of not having the full source code :(\n");
			g_testPostProcess.SetString("");
		}
		else {
			renderSystem->DrawStretchPic(0.0f, 0.0f, SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f, shiftScale.y, shiftScale.x, 0.0f, mtr);
		}
	}
// jmarshall end
